		...
		<div class="attr" :style='{"padding":"0","overflow":"hidden","flexWrap":"wrap","background":"none","display":"flex","width":"100%","position":"relative","justifyContent":"space-between"}'>
			...
			<div class="info" :style='{"minHeight":"500px","width":"44%","padding":"20px 0","margin":"0","background":"none"}'>
				...
				<div class="item" :style='{"border":"0 dotted #bfe7e3","padding":"0","margin":"0 0 10px 0","background":"none","borderWidth":"0 0 2px","display":"flex","justifyContent":"spaceBetween"}'>
					<div class="lable" :style='{"padding":"0 10px","color":"#992298","textAlign":"right","width":"auto","fontSize":"14px","lineHeight":"40px","minWidth":"120px","height":"40px"}'>文章编号</div>
					<div  :style='{"padding":"8px 10px 0","color":"#666","background":"none","width":"100%","fontSize":"14px","lineHeight":"24px","height":"auto"}'>{{detail.wenzhangbianhao}}</div>
				</div>
				<div class="item" :style='{"border":"0 dotted #bfe7e3","padding":"0","margin":"0 0 10px 0","background":"none","borderWidth":"0 0 2px","display":"flex","justifyContent":"spaceBetween"}'>
					<div class="lable" :style='{"padding":"0 10px","color":"#992298","textAlign":"right","width":"auto","fontSize":"14px","lineHeight":"40px","minWidth":"120px","height":"40px"}'>文章分类</div>
					<div  :style='{"padding":"8px 10px 0","color":"#666","background":"none","width":"100%","fontSize":"14px","lineHeight":"24px","height":"auto"}'>{{detail.wenzhangfenlei}}</div>
				</div>
				<div class="item" :style='{"border":"0 dotted #bfe7e3","padding":"0","margin":"0 0 10px 0","background":"none","borderWidth":"0 0 2px","display":"flex","justifyContent":"spaceBetween"}'>
			...
			<div class="hot" :style='{"boxShadow":"0 0px 0px rgb(0 0 0 / 10%)","padding":"150px 7% 40px","margin":"20px 0 0","background":"url(http://codegen.caihongy.cn/20230130/d81fd2d44f4e475eb1c571b1f64e5fae.png) no-repeat center top","width":"100%","position":"relative","height":"auto"}'>
			  ...
			  <div :style='{"padding":"20px","margin":"20px 0 0","flexWrap":"wrap","background":"none","display":"flex","width":"100%","justifyContent":"space-between","height":"auto"}'>
			    <div v-for="item in hotList" :key="item" :style='{"width":"23%","padding":"0 0 10px","margin":"0 0 10px","borderRadius":"8px","background":"#fff","height":"auto"}' @click="toDetail(item)">
			      <img :style='{"padding":"20px","margin":"0 0 10px","objectFit":"cover","borderRadius":"8px 8px 0 0","background":"url(http://codegen.caihongy.cn/20230131/6afa6fc6db7b4b62bd4e8f4a24f3975e.png) no-repeat center top / 100% 100%","display":"block","width":"100%","height":"280px"}' :src="baseUrl + (item.wenzhangfengmian?item.wenzhangfengmian.split(',')[0]:'')" alt="">
			      <div :style='{"padding":"4px 0","margin":"0 auto 10px","color":"#333","textAlign":"center","background":"linear-gradient(0deg, rgba(238,177,238,1) 0%, rgba(165,228,222,1) 30%, rgba(165,228,222,1) 100%)","width":"90%","lineHeight":"30px","fontSize":"14px"}'>{{item.wenzhangbiaoti}}</div>
    ...
    methods: {
        init() {
          ...
          this.$http.get(this.tablename + '/detail/'  + this.detail.id, {}).then(res => {
            if (res.data.code == 0) {
              ...
              this.detailBanner = this.detail.wenzhangfengmian ? this.detail.wenzhangfengmian.split(",") : [];
              this.$forceUpdate();
            }
          });
      ...
      getStoreupStatus(){
        if(localStorage.getItem("Token")) {
            this.$http.get('storeup/list', {params: {page: 1, limit: 1, type: 1, refid: this.detail.id, tablename: 'jiaoyuwenzhang', userid: localStorage.getItem('userid')}}).then(res => {
      ...
      getDiscussList(page) {
        this.$http.get('discussjiaoyuwenzhang/list', {params: {page, limit: this.pageSize, refid: this.detail.id}}).then(res => {
          if (res.data.code == 0) {
      ...
      submitForm(formName) {
        ...
        this.$refs[formName].validate((valid) => {
          if (valid) {
            ...
            this.$http.post('discussjiaoyuwenzhang/add', this.form).then(res => {
              if (res.data.code == 0) {
                this.form.content = '';
                this.getDiscussList(1);
                this.$message({
